...
export default async function handler(req, res) {
  ...
  if (mandateId) {
    try {
      const [live, m] = await Promise.all([
        publicClient.readContract({ address: executor, abi: AGENT_EXECUTOR_ABI, functionName: 'isMandateLive', args: [mandateId] }),
        publicClient.readContract({ address: executor, abi: AGENT_EXECUTOR_ABI, functionName: 'mandates', args: [mandateId] }),
      ]);
      ...
      return res.status(200).json({ mandateId, live: false, note: err?.shortMessage || err?.message });
    }
  }
  ...
  try {
    ...
    return res.status(200).json({
      ...
      mandateId: result?.proposalId || result?.mandate_id || null,
