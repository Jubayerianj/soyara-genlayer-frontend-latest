...
//
// The server's record of a queued trade: this server's file, or the separate
// settlement server when SETTLEMENT_SERVER_URL is set (lib/settlementBackend.js).
//
...
import { getTrade, cancelTrade } from '../../lib/settlementBackend.js';
...
export default async function handler(req, res) {
  if (req.method === 'GET') {
    ...
    const e = await getTrade(commitment);
  ...
  if (req.method === 'POST') {
    ...
    const e = await cancelTrade(cancel);
