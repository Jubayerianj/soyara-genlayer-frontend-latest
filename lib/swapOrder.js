...
import { nativeInputReason } from './nativeInput.js';
...
export function normaliseSwapIntent(proposal) {
  ...
  // on finalization only. On Bradbury that is 30 minutes after the round's
  // last vote. The old 20-minute default therefore expired before settlement was
...
export async function buildSwapOrder({
  ...
  // Native GEN in would be paid for by the relayer, not the user: AgentExecutor
  // can only pull ERC-20s. Refused before any quote or round (lib/nativeInput.js).
  if (tokenInAddr === zeroAddress) {
    return { ok: false, status: 400, body: { error: nativeInputReason(), wrapFirst: true } };
  }
