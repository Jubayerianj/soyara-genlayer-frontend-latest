...
export const notices = {
  ...
  queued: (commitment, label) => notify({
    id: `settle:${commitment}`, kind: 'pending', title: `Settling · ${label}`, body: 'About 30 min. Settles by itself, even if you close Soyara.',
  }),
  waitingBehind: (id, label, ahead) => notify({
    id: `settle:${id}`, kind: 'pending', title: `Settling · ${label}`,
    body: `Waiting on ${ahead} earlier round${ahead === 1 ? '' : 's'}. It finishes by itself.`,
  }),
  nextInLine: (id, label) => notify({
    id: `settle:${id}`, kind: 'pending', title: `Settling · ${label}`, body: 'Next in line. It finishes by itself.',
  }),
