...
/**
 * An account that signs GenLayer consensus writes with gas headroom.
 *
 * genlayer-js signs every ConsensusMain write with exactly what eth_estimateGas
 * returned. What addTransaction really needs moves from block to block, because
 * it draws the round's validators: one swap round needed 1,174,804 to 1,187,109
 * gas across twelve Bradbury blocks, against an estimate of 1,176,262. So a
 * limit equal to the estimate reverts whenever the including block needs a
 * little more, and the round never starts. On 11 Sep 2026 that happened three
 * submissions running ("GenLayer did not accept the proposal"). The
 * finalization calls draw validators too.
 *
 * genlayer-js hands the transaction, gas included, to the account to sign, so
 * the account is where the headroom goes. Unused gas is refunded, and at
 * Bradbury's gas price the higher limit costs nothing.
 */
export const GAS_HEADROOM_PCT = 130n;

export function withGasHeadroom(account) {
  if (!account || account.type !== 'local' || typeof account.signTransaction !== 'function' || account.gasHeadroom) return account;
  return {
    ...account,
    gasHeadroom: true,
    signTransaction: (tx, opts) => account.signTransaction(
      tx?.gas ? { ...tx, gas: (BigInt(tx.gas) * GAS_HEADROOM_PCT) / 100n } : tx,
      opts,
    ),
  };
}
...
export async function drainFinalizationQueue({
  ...
  for (let step = 0; step < maxSteps; step += 1) {
    ...
    try {
      const signer = withGasHeadroom(account);
      if (action === 'finalize') await client.finalizeTransaction({ account: signer, txId: head.txId });
      else await client.finalizeIdlenessTxs({ account: signer, txIds: [head.txId] });
...
async function _consensusRound(functionName, args, options, action = undefined, commitment = null) {
  ...
  try {
    txHash = await client.writeContract({
      account: withGasHeadroom(options.account),
...
export async function finalizeStuckValidation(txHash, account) {
  ...
  try {
    const evmTxHash = await client.finalizeIdlenessTxs({ account: withGasHeadroom(account), txIds: [txHash] });
