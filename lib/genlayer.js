...
export function shouldNudgeFinalize(txHash, now = Date.now()) {
  if (!txHash) return false;
  const seen = _nudges.get(txHash) || { firstSeen: now, lastAttempt: 0 };
...
export async function finalizeRound(txHash, account) {
  ...
  if (!shouldNudgeFinalize(txHash)) return false;
