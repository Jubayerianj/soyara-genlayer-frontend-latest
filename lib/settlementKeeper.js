...
// routes as a fallback); any open Soyara tab also pings it once a minute. A
// host that cannot keep a timer running uses the separate settlement server
// instead (SETTLEMENT_SERVER_URL, lib/settlementBackend.js), and this keeper
// then stays off.
...
import { usesSettlementServer } from './settlementBackend.js';
...
export async function settlementKeeperPass({
  ...
  for (const e of list()) {
    ...
    try {
      ...
      // this pass looks at it (a server that slept past the deadline).
...
// Only the three reads the keeper makes, inline: reading abi/ from disk at run
// time failed wherever the app ships without its source tree (the Dockerfile
// copies .next, not abi/), and every read then looked like "no verdict yet".
const EXECUTOR_READS = parseAbi([
  'function commitmentUsed(bytes32) view returns (bool)',
  'function isVerdictLive(bytes32) view returns (bool)',
  'function verdictExpiry(bytes32) view returns (uint64)',
]);
...
export function realKeeperDeps() {
  ...
  const read = (functionName, commitment) => chainClient().readContract({ address: executor, abi: EXECUTOR_READS, functionName, args: [commitment] });
 ...
 * Not at all when a separate settlement server holds the trades: it runs the
 * keeper, and a second one here would race it.
 */
export function ensureSettlementKeeper() {
  if (typeof window !== 'undefined' || globalThis.__soyaraSettlementKeeper || usesSettlementServer()) return;
