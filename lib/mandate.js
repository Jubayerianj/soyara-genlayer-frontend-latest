...
export function rememberMandate(user, tokenIn, tokenOut, mandateId, roundTxHash = null) {
  if (!mandateId) return;
  const all = readAll();
  ...
  // a round on its own.
  all[slot(user, tokenIn, tokenOut)] = { mandateId, roundTxHash, at: Date.now() };
  writeAll(all);
}
...
export async function findUsableMandate({ user, tokenIn, tokenOut, amountIn }) {
  ...
  try {
    const res = await fetch('/api/agent-mandate', {
      ...
      // roundTxHash makes this check drive finalization too, not just observe it.
      body: JSON.stringify({
        mandateId, checkOnly: true,
        roundTxHash: recallRound(user, tokenIn, tokenOut)?.roundTxHash || null,
        roundSubmittedAt: recallRound(user, tokenIn, tokenOut)?.at || null,
      }),
    });
