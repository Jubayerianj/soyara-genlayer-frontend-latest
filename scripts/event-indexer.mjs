/**
 * scripts/event-indexer.mjs
 * 
 * Production-ready Event Indexer Daemon for LitVM Dex Swaps.
 * Uses Viem's watchContractEvent (supporting WebSockets or HTTP Polling fallback)
 * to index AGGFlowSwap events into MongoDB in real-time.
 * 
 * Includes:
 * 1. Catch-up (backfill) phase on startup to fetch missed blocks since last run.
 * 2. Real-time event streaming via WebSockets/polling.
 * 3. Self-healing / Auto-reconnection logic.
 * 4. Synchronization with the existing MongoDB transaction & user_stats schemas.
 * 
 * Run using:
 * node scripts/event-indexer.mjs
 */
...
const LitVM = {
  ...
  rpcUrls: {
    default: {
      ...
      webSocket: ['wss://liteforge.rpc.caldera.xyz/infra-partner-ws'], // Replace with actual WS endpoint if available
    },
  },
};
...
// Update user_stats collection matching /api/track.js logic
...
async function fetchTokenPriceUSD(symbol, publicClient) {
  try {
    ...
    return Number(formatUnits(answer, 18));
  } catch (error) {
...
async function processLog(db, log, publicClient) {
  ...
  // Determine USD volume using price oracle
  let usdValue = 0;
  try {
  ...
  // Construct transaction document matching the Next.js frontend schema
  const transactionDoc = {
    ...
    fromToken: tIn,
    toToken: tOut,
    amount: formattedIn, // quantity sold
    usdValue: usdValue,
    status: 'completed',
  ...
  // Upsert transaction document to prevent duplicates
  ...
  if (txRes.upsertedCount > 0) {
    ...
    // Update user stats aggregates
    await updateUserStats(db, user, transactionDoc);
  } else {
    console.log(`ℹ️  Transaction already indexed: ${txHash}`);
  }

  // Update synchronization state to the latest block number processed
...
// Catch up / backfill historical blocks
async function catchUpHistorical(db, publicClient, currentBlock) {
  ...
  const CHUNK_SIZE = 10000; // Batch limit to prevent RPC rate limits
  ...
  while (blockCursor <= currentBlock) {
    ...
    try {
      ...
      // Update sync state block number at the end of each chunk
      await db.collection('sync_state').updateOne(
        { key: 'last_processed_block' },
...
async function runIndexer() {
  if (!MONGODB_URI) {
    console.error('❌ MONGODB_URI is missing. Please set it in .env or .env.local.');
    process.exit(1);
  }
  ...
  // Setup WebSocket with fallback HTTP transport
  ...
  if (wsRpcUrl) {
    ...
    console.log(`🔌 WebSocket URL not specified (LITVM_WS_RPC_URL). Using HTTP Polling transport: ${httpRpcUrl}`);
  ...
  // Catch up on any missing blocks first
  ...
  const unwatch = publicClient.watchContractEvent({
    ...
    eventName: 'AGGFlowSwap',
    onLogs: async (logs) => {
      console.log(`🔔 Received ${logs.length} new log(s) from subscription`);
      for (const log of logs) {
        try {
          await processLog(db, log, publicClient);
        } catch (err) {
    ...
    onError: (error) => {
      ...
      // Restart the indexer to trigger re-connection and catch-up
