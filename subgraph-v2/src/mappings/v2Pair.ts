...
import { LiquidityProvision, LiquidityRemoval, V2Burn, V2Mint, V2Pair, V2Swap } from '../../generated/schema';
import {
  ONE_BI,
...
export function handleSwap(event: Swap): void {
  ...
  let isAgainstNative = (pair.token0.toLowerCase() == WRAPPED_NATIVE_ADDRESS) || (pair.token1.toLowerCase() == WRAPPED_NATIVE_ADDRESS);
  if (isAgainstNative) {
    if (pair.token0.toLowerCase() != WRAPPED_NATIVE_ADDRESS) {
      trackV2VolumeAgainstNative(token0Entity, rawVolume0, event.block.timestamp);
    }
    if (pair.token1.toLowerCase() != WRAPPED_NATIVE_ADDRESS) {
      trackV2VolumeAgainstNative(token1Entity, rawVolume1, event.block.timestamp);
    }
  }
...
export function handleBurn(event: Burn): void {
  ...
  liquidityRemoval.protocolVersion = 'V2';
  liquidityRemoval.user = sender.id;
  liquidityRemoval.marketId = pair.id;
